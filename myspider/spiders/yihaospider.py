...
class yihaoSpider(scrapy.Spider):
	...
	def parse_item(self,response):
		...
		store_link=response.urljoin(response.css('.key_shop>a::attr("href")').extract_first())
		...
		item.add_css('commentsnum','#div_peheader::attr("pecount")',TakeFirst())
		item.add_css('goodcomments','.mod_circle>div>p>span::text',TakeFirst())
		item.add_css('goodcommentnum','li[tag="good-comment"]>span::text',TakeFirst())
		item.add_css('mediumcommentnum','li[tag="general-comment"]>span::text',TakeFirst())
		item.add_css('badcommentnum','li[tag="bad-comment"]>span::text',TakeFirst())
		item.add_css('comment_tags','#div_labelSummary>div>a::text')
		item.add_css('store','.key_shop>a::text',TakeFirst())
		item.add_value('store_link',store_link)
