...
import logging
from myspider.items import JdlistItem,JdItem,TaobaolistItem,TaobaoItem,YihaoItem,YihaoList
...
class InsertMysql(object):
	...
	def process_item(self,item,spider):
		if isinstance(item,JdlistItem):
		...
		elif isinstance(item,YihaoList):
			...
			self.cursor.execute('insert into yihaoitem(id,name,price,brand,parameter,summary_service,url,sales_promotion,add_service,store_link,commentsnum,goodcomments,comment_tags,goodcommentnum,mediumcommentnum,badcommentnum,store,images_url) values("%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s")' % (item['id'],item['name'],item['price'],item['brand'],item['parameter'],item['summary_service'],item['url'],item['sales_promotion'],item['add_service'],item['store_link'],item['commentsnum'],item['goodcomments'],item['comment_tags'],item['goodcommentnum'],item['mediumcommentnum'],item['badcommentnum'],item['store'],item['images_url']))
